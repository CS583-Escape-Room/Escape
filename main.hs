...
run_code = do
        putStrLn ""
        putStrLn ("Now you are in the " ++ (get_name (get_player_room player1 house)) ++ ".")
        ...
        if line == "help" then help "run_code"
                                            ...
                                            putStrLn ("The objects in room: " ++ (intercalate ", " (map get_name (get_objects (get_player_room player1 house)))))
                                            run_code
        else if line == "check bag" then do 
                                            let i = (get_bag_item player1)
